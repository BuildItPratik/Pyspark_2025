...
echo " Exporting CSV from HDFS..."
  # Fix permissions INSIDE a container
  docker exec spark-master chmod -R 777 /opt/spark-apps/

  # Create output directory INSIDE the container
  docker exec spark-master mkdir -p "/opt/spark-apps/shared_output/customer_etl/${RUN_DATE}"
